...
- `shared.flagids_format`: flag IDs structure

## Operational notes

- If `url_flag_checker` is empty, server stores flags but cannot forward them.
- `flag_ttl` is in ticks (`flag_ttl * tick_time` seconds).
- `flagids_format`: flag IDs structure that defines how flags are formatted (e.g., `[service].[team].[id]` means flags follow a pattern with service name, team number, and flag ID separated by dots)
